...
export const ProductDetailModal: React.FC<ProductDetailModalProps> = ({
  ...
  // Media View Mode (Images vs Video)
      ...
      <div className="bg-white w-full max-w-4xl rounded-2xl shadow-2xl overflow-hidden relative my-auto max-h-[92vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto scroll-smooth divide-y divide-slate-100">
          ...
          <div className="space-y-4">
            ...
            {/* Media Screen Box */}
            <div className="aspect-square w-full rounded-xl bg-slate-900 overflow-hidden border border-slate-200 relative shadow-inner flex items-center justify-center">
            ...
            <div className="flex items-center gap-2 overflow-x-auto pb-1">
              {product.videoUrl && (
                <button
                  type="button"
                  onClick={() => setMediaMode('video')}
                  className={`w-16 h-16 rounded-lg overflow-hidden border-2 transition-all flex flex-col items-center justify-center bg-slate-900 text-white shrink-0 cursor-pointer ${
                    mediaMode === 'video' ? 'border-[#FF5500] ring-2 ring-orange-200' : 'border-transparent opacity-80 hover:opacity-100'
                  }`}
                  title="Watch Video"
                >
                  <Play className="w-5 h-5 text-[#FF5500] fill-[#FF5500]" />
                  <span className="text-[9px] font-black tracking-wider text-white uppercase mt-0.5">Video</span>
                </button>
              )}
              ...
              {images.map((img, idx) => (
                <button
                  ...
                  className={`w-16 h-16 rounded-lg overflow-hidden border-2 transition-all shrink-0 cursor-pointer ${
                    mediaMode === 'image' && selectedImage === img
                      ? 'border-[#FF5500]'
              ...
              {product.tags && product.tags.length > 0 && (
                <div className="flex flex-wrap items-center gap-1.5 pt-1">
                  ...
                  {product.tags.map((tag) => (
                    <span
                      key={tag}
                      className="px-2 py-0.5 bg-orange-50 border border-orange-200 text-[#FF5500] text-[11px] font-extrabold rounded-md shadow-2xs"
                    >
                      #{tag}
                    </span>
                  ))}
